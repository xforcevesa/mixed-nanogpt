...
class CausalSelfAttention(nn.Module):
    ...
    def update_cluster(self, q, k, v):
        B, NH, QT, HS = q.size()
        KT, VT = k.size(2), v.size(2)
        assert KT == VT, "key and value must have the same length"
        if KT <= self.n_cluster:
            return k, v
        else:
            attn_weights = torch.matmul(q, k.transpose(2, 3)) / math.sqrt(HS) # (B, NH, QT, KT)
            ...
            pool_vote = F.avg_pool1d(vote, kernel_size = self.kernel_size, padding=self.kernel_size//2, stride=1)
            indices = pool_vote.topk(self.n_cluster, dim=-1).indices
            # Expand the indices to match the head dimension for gathering
            indices = indices.unsqueeze(-1).expand(-1, -1, -1, HS)
            k_cluster = torch.gather(k, 2, indices)
            v_cluster = torch.gather(v, 2, indices)
            return k_cluster, v_cluster
    ...
    def long_forward(self, x):
        ...
        for i in range(n_group):
            q_cur = q[:, i, :, :].view(B, self.group_size, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, gs, hs)
            k_cur = k[:, i, :, :].view(B, self.group_size, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, gs, hs)
            v_cur = v[:, i, :, :].view(B, self.group_size, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, gs, hs)
            # 
            if k_past is not None and v_past is not None:
                k_past_compress, v_past_compress = self.update_cluster(q_cur, k_past, v_past)
                key = torch.cat([k_past_compress, k_cur], dim=2)
                ...
                value = v_cur
                # create causal mask
                causal_mask = torch.ones(self.group_size, self.group_size, dtype=torch.bool, device=device).tril(diagonal=0) # (gs, gs)
            # calculate attention
            yi = F.scaled_dot_product_attention(q_cur, key, value, attn_mask=causal_mask) # flash attention
...
class GPTConfig:
    ...
    kernel_size: int = 5 # kernel size for pooling
...
log_dir = "log/lmh_L12D768_CTX1024_C256G256_INTV4_snap"
