...
@dataclass
class GPTConfig:
    block_size: int = 4000 # max sequence length
    vocab_size: int = 50257 # number of tokens: 50,000 BPE merges + 256 bytes tokens + 1 <|endoftext|> token
    n_layer: int = 12 # number of layers
    n_head: int = 12 # number of heads
    n_embd: int = 768 # embedding dimension
    moba_chunk_size: int = 100 # chunk_size
    moba_topk: int = 3 # top-k blocks to attend
    chunk_len: int = 16 # don't change
    interval: int = 13 # 1 Transformer block every n blocks
...
T = GPTConfig().block_size # sequence length
...
config = GPTConfig()
log_dir = f"log/lhm_L{config.n_layer}D{config.n_embd}_CTX{config.block_size}_CS{config.moba_chunk_size}TOPK{config.moba_topk}_INTV{config.interval}"
#log_dir = "log/lhm_L12D768_CTX1024_CS128TOPK3_INTV12"
...
for step in range(max_steps):
    ...
    if step % 250 == 0 or last_step:
        ...
        if master_process:
            ...
            with open(log_file, "a") as f:
                f.write(f"{step} {step*total_batch_size} val {val_loss_accum:.4f} {dt_per_step*1000:.2f}ms\n")
            break
    ...
    mem_alloc = torch.cuda.memory_allocated(device) / 1024 ** 3 # in GiB
    if master_process:
        ...
        with open(log_file, "a") as f:
            f.write(f"{step} {step*total_batch_size} train {loss_accum:.6f} {dt*1000:.2f}ms {mem_alloc:.2f}GiB\n")
